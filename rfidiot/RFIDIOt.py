...
class rfidiot:
        ...
        if not self.NoInit:
            if self.readertype == self.READER_PCSC:
                ...
                if self.readername.find("OMNIKEY") == 0:
                    ...
                    if str.find(self.readername, "SDI010") == 0:
                        self.readersubtype = self.READER_SCM
                    else:
                        if str.find(self.readername, "ACS ACR122U PICC") == 0:
                            self.readersubtype = self.READER_ACS
                            self.pcsc_protocol = smartcard.scard.SCARD_PROTOCOL_T1
                            self.hcard = None
                        elif str.find(self.readername, "ACS ACR1255U-J1 PICC") == 0:
                            self.readersubtype = self.READER_PCSC
                            self.pcsc_protocol = smartcard.scard.SCARD_PROTOCOL_T1
                            self.hcard = None
                        elif str.find(self.readername, "ACS") == 0:
                            self.readersubtype = self.READER_ACS
                ...
                try:
                    ...
                    if self.readersubtype == self.READER_ACS and self.pcsc_protocol == smartcard.scard.SCARD_PROTOCOL_T1:
                        ...
                        hresult, hcard, _dwActiveProtocol = smartcard.scard.SCardConnect(
                            ...
                            smartcard.scard.SCARD_PROTOCOL_T0,
                        )
                        self.hcard = hcard
    ...
    # Static Globals
    MIFAREACKB = {
        "000": "Write KeyA: KEYA, Read Access bits: KEYA, Write Access bits: NONE, Read KeyB: KEYA, Write KeyB: KEYA (KEYB readable)",
    ...
    MIFAREACDB = {
        "000": "Read: KEYA/B, Write: KEYA/B, Increment: KEYA/B, Decrement/Transfer/Restore: KEYA/B (transport configuration)",
    ...
    LFXTagBlocks = {"U": 0, "Z": 0, "T": 34, "h": 64, "H": 8, "Q": 8, "R": 18, "N": 0}
    ALL = "all"
    EM4x02 = "U"
    EM4x05 = "Z"
    Q5 = "Q"
    ...
    ISO_SPEED = {"00": "106kBaud", "02": "212kBaud", "04": "424kBaud", "08": "848kBaud"}
    ISO_FRAMESIZE = {
        "00": "16",
        "01": "24",
        "02": "32",
    ...
    def readEEPROM(self, byte) -> str:
        self.ser.write("rp%02x" % byte)
        return self.ser.readline()[:2]

    def writeEEPROM(self, byte, value) -> bool:
    ...
    def settagtype(self, ttype) -> bool:
        if self.readertype == self.READER_ACG:
            # ACG HF reader uses 't' for 'all', LF uses 'a'
            if ttype == self.ALL:
                if str.find(self.readername, "LFX") == 0:
                    ttype = "a"
                else:
                    ttype = "t"
            self.ser.write("o" + ttype)
            self.errorcode = self.ser.readline()[:-2]
            if self.errorcode == "O" + ttype.upper():
                self.tagtype = ttype
                return True
        if self.readertype == self.READER_FROSCH:
            if ttype == self.EM4x02:
                return self.frosch(self.FR_HT2_Read_Miro, "")
            if ttype == self.EM4x05:
    ...
    # card functions
    #
    def pcsc_listreaders(self) -> None:
        n = 0
        print("PCSC devices:")
    ...
    def waitfortag(self, message) -> bool:
        if message:
            print(message, end=" ")
            sys.stdout.flush()
    ...
    def h2login(self, password) -> bool:
    ...
    def acs_send_direct_apdu(self, apdu) -> bool:
    ...
    def acs_read_block(self, block) -> bool:
        "ACS READ Block"
        readblock = "%02x" % block
        read = False
    ...
    def acs_get_sam_id(self) -> bool:
        "ACS get SAM id"
        return self.acs_send_apdu(self.PCSC_APDU["ACS_GET_SAM_ID"])

    def acs_set_retry(self, time) -> bool:
        "ACS set retry"
    ...
    def acs_get_firmware_revision(self) -> bool:
    ...
    def gp_initialize_update_response_scp02(self, data) -> bool:
    ...
    def pcsc_send_apdu(self, apdu) -> bool:
    ...
    def login_iclass(self, page, keynum) -> bool:
    ...
    def login(self, sector, keytype, key) -> bool:
    ...
    def authenticate(self, sector, keytype, keynum) -> bool:
        ##- keytype= string.upper(keytype)
        keytype = keytype.upper()
        apdu = []
    ...
    def verify(self, keytype, key) -> bool:
        ##- keytype= string.upper(keytype)
        keytype = keytype.upper()
        apdu = []
    ...
    def writevalueblock(self, block, data) -> bool:
    ...
    def frosch(self, command, data) -> bool:
    ...
    def frosch_bcc_in(self, data, seed) -> int:
        return self.frosch_bcc(data[: len(data) - 1], seed)

    def frosch_bcc_out(self, data, seed) -> int:
    ...
    def demotag(self, command, data) -> bool:
    ...
    def MACVerify(self, message, key) -> bool:
    ...
    def SSCIncrement(self, ssc) -> str:
        out = int(self.ToHex(ssc), 16) + 1
        return self.ToBinary("%016x" % out)

    def TRANSITIDEncode(self, data) -> str:
        "Encode TRANSIT ID"
        # start sentinel
    ...
    def TRANSITIDPrint(self, data) -> str:
    ...
    def FDXBIDEncode(self, appid, ccode, natid) -> str:
        "Encode FDX-B ID"
    ...
    def FDXBID128Bit(self, data) -> str:
    ...
    def PCSCGetTagType(self, atr) -> str:
